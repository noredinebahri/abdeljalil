...
import {IDevise} from "../../../../models/referentiel/devise";
...
export class DeviseComponent {
  ...
  constructor(private modalService: NgbModal, private referentielBaseService: ReferentielBaseService<IDevise>,
              private toastService: ToastService, private translateService: TranslateService) {
    this.referentielBaseService.setUrlEndpoint('Devise');
  }
  ...
  loadData(page?:number){
    ...
    this.referentielBaseService.getAll(queryParams).subscribe({
      next: (data:HttpResponse<IDevise[]>)=>{
        if(data.body)
          this.onSuccess(data.body, data.headers, pageSearch);
      }
    });
  }
  protected onSuccess(res: IDevise[], headers: HttpHeaders, page: number): void {
  ...
  openUpdateModal(item?:IDevise,action?:string) {
    ...
    modalRef.result.then((form) => {
      const devise: IDevise = form.value;
      devise.id = item?.id;
      devise.active = item?.active;
      this.referentielBaseService.createOrUpdate(devise)
        .subscribe({
          next: (res) => {
  ...
  setFormItemValues(item?:IDevise):FormItem[]{
    if(item){
      this.depotFormItems = [
        {
          itemType: 'field',
