...
import {INatureIdentifiantDeposant} from "../../../../models/referentiel/nature-identifiant-deposant";
...
@Component({
  selector: 'app-nature-identifiant-deposant',
  templateUrl: './nature-identifiant-deposant.component.html',
  styleUrl: './nature-identifiant-deposant.component.scss'
})
export class NatureIdentifiantDeposantComponent {
  ...
  constructor(private modalService: NgbModal, private referentielBaseService: ReferentielBaseService<INatureIdentifiantDeposant>,
              private toastService: ToastService, private translateService: TranslateService) {
    this.referentielBaseService.setUrlEndpoint('NatureIdentifiantDeposant');
  }
  ...
  tableData: INatureIdentifiantDeposant[] = []
  ngOnInit() {
    this.loadData();
  }
  ...
  loadData(page?:number){
    ...
    this.referentielBaseService.getAll(queryParams).subscribe({
      next: (data:HttpResponse<INatureIdentifiantDeposant[]>)=>{
        if(data.body)
          this.onSuccess(data.body, data.headers, pageSearch);
      }
    });
  }
  protected onSuccess(res: INatureIdentifiantDeposant[], headers: HttpHeaders, page: number): void {
  ...
  openUpdateModal(item?:INatureIdentifiantDeposant,action?:string) {
    ...
    modalRef.result.then((form) => {
      const natureIdentifiantDeposant: INatureIdentifiantDeposant = form.value;
      natureIdentifiantDeposant.id = item?.id;
      natureIdentifiantDeposant.active = item?.active;
      this.referentielBaseService.createOrUpdate(natureIdentifiantDeposant)
        .subscribe({
          next: (res) => {
  ...
  setFormItemValues(item?:INatureIdentifiantDeposant):FormItem[]{
    if(item){
      this.depotFormItems = [
        {
          itemType: 'field',
          ...
          type: 'text',
          value: "natureIdentifiantDeposantTitle",
          disabled: true,
        },
        {
          itemType: 'field',
