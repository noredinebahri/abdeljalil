...
import {IFormeJuridique} from "../../../../models/referentiel/forme-juridique";
import {
  ActionClickEvent,
  ActionConfig,
  ColumnConfig,
...
@Component({
  selector: 'app-nationalite-pays-naissance',
  templateUrl: './nationalite-pays-naissance.component.html',
  styleUrl: './nationalite-pays-naissance.component.scss'
})
export class NationalitePaysNaissanceComponent {
  ...
  constructor(private modalService: NgbModal, private referentielBaseService: ReferentielBaseService<IFormeJuridique>,
              private toastService: ToastService, private translateService: TranslateService) {
    this.referentielBaseService.setUrlEndpoint('FormeJuridique');
  }
  ...
  loadData(page?:number){
    ...
    this.referentielBaseService.getAll(queryParams).subscribe({
      next: (data:HttpResponse<IFormeJuridique[]>)=>{
        if(data.body)
          this.onSuccess(data.body, data.headers, pageSearch);
      }
    });
  }
  protected onSuccess(res: IFormeJuridique[], headers: HttpHeaders, page: number): void {
  ...
  openUpdateModal(item?:IFormeJuridique,action?:string) {
    ...
    modalRef.result.then((form) => {
      const formeJuridique: IFormeJuridique = form.value;
      formeJuridique.id = item?.id;
      formeJuridique.active = item?.active;
      this.referentielBaseService.createOrUpdate(formeJuridique)
        .subscribe({
          next: (res) => {
  ...
  setFormItemValues(item?:IFormeJuridique):FormItem[]{
    if(item){
      this.depotFormItems = [
        {
          itemType: 'field',
