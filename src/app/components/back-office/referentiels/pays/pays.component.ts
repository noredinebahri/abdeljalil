...
import {IPays} from "../../../../models/referentiel/pays";
...
export class PaysComponent {
  ...
  constructor(private modalService: NgbModal, private referentielBaseService: ReferentielBaseService<IPays>,
              private toastService: ToastService, private translateService: TranslateService) {
    this.referentielBaseService.setUrlEndpoint('pays');
  }

  columns: ColumnConfig[] = [
    { field: 'code', header: 'referentiel.code', type: 'text'},
    { field: 'libelle', header: 'referentiel.libelle', type: 'text'},
    { field: 'nationalite', header: 'referentiel.nationalite', type: 'text'},
  ...
  filterFields: FilterField[] = [
    ...
    { name: 'nationalite', placeholder: 'referentiel.nationalite', type: 'text' },
  ...
  loadData(page?:number){
    ...
    this.referentielBaseService.getAll(queryParams).subscribe({
      next: (data:HttpResponse<IPays[]>)=>{
        if(data.body)
          this.onSuccess(data.body, data.headers, pageSearch);
      }
    });
  }
  protected onSuccess(res: IPays[], headers: HttpHeaders, page: number): void {
  ...
  openUpdateModal(item?:IPays,action?:string) {
    ...
    modalRef.result.then((form) => {
      const pays: IPays = form.value;
      pays.id = item?.id;
      pays.active = item?.active;
      this.referentielBaseService.createOrUpdate(pays)
        .subscribe({
          next: (res) => {
  ...
  setFormItemValues(item?:IPays):FormItem[]{
    if(item){
      this.depotFormItems = [
        {
          itemType: 'field',
