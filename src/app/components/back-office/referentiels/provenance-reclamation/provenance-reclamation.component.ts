...
import { IPRovenanceReclamation } from 'src/app/models/referentiel/ProvenaceReclamation';

@Component({
  selector: 'app-provenance-reclamation',
  templateUrl: './provenance-reclamation.component.html',
  styleUrl: './provenance-reclamation.component.scss'
})
export class ProvenanceReclamationComponent {
  ...
  constructor(private modalService: NgbModal, private referentielBaseService: ReferentielBaseService<IPRovenanceReclamation>,
              private toastService: ToastService, private translateService: TranslateService) {
    this.referentielBaseService.setUrlEndpoint('ProvenanceReclamation');
  }
  ...
  tableData: IPRovenanceReclamation[] = []
  ngOnInit() {
    this.loadData();
  }
  ...
  loadData(page?: number, filters?: any): void {
    ...
    this.referentielBaseService.getAll(queryParams).subscribe({
      next: (data: HttpResponse<IPRovenanceReclamation[]>) => {
        if (data.body) {
  ...
  protected onSuccess(res: IPRovenanceReclamation[], headers: HttpHeaders, page: number): void {
  ...
  openUpdateModal(item?:IPRovenanceReclamation,action?:string) {
    ...
    modalRef.result.then((form) => {
      const agentPayer: IPRovenanceReclamation = form.value;
  ...
  setFormItemValues(item?:IPRovenanceReclamation):FormItem[]{
    if(item){
      this.depotFormItems = [
        {
          itemType: 'field',
